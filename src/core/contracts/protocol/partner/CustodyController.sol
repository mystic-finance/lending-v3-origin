 ...
 */
contract CustodyController is AccessControl, ReentrancyGuard, Pausable {
   ...
   */
  function batchGrantWithdrawalOperatorRole(
    address[] calldata _newWithdrawalOperators
  ) external onlyRole(TREASURY_MANAGER_ROLE) {
    for (uint256 i = 0; i < _newWithdrawalOperators.length; i++) {
      ...
      _setupRole(WITHDRAWAL_OPERATOR_ROLE, operator);
    }
  }
   ...
   */
  function revokeWithdrawalOperatorRole(
    address _withdrawalOperator
  ) external onlyRole(TREASURY_MANAGER_ROLE) {
