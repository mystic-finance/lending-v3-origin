...
contract LeveragedBorrowingVault02 is Ownable, ReentrancyGuard, IFlashLoanReceiver {
  ...
  uint256 public SLIPPAGE_TOLERANCE = 100; // 0.5%
  uint24 public DEFAULT_POOL_FEE = 50; // 0.1% pool fee
  ...
  mapping(bytes => uint256) public mapPositions;
  mapping(address => mapping(uint256 => uint256)) public userPositionIndex;
  ...
  function openLeveragePosition(
    ...
    bool isCollateralAllowed = allowedCollateralTokens[collateralToken];
    bool isBorrowAllowed = allowedBorrowTokens[borrowToken];
    require(isCollateralAllowed, 'Collateral token not allowed');
    require(isBorrowAllowed, 'Borrow token not allowed');
    require(leverageMultiplier > 1 && leverageMultiplier <= MAX_LEVERAGE, 'Invalid leverage');

    require(
      mapPositions[encodeAddresses(msg.sender, collateralToken, borrowToken)] <= 0,
      'Position for this pair already exists for this user'
    );
    ...
    flashLoanController.executeFlashLoan(
      borrowToken,
      expectedAmountIn,
      abi.encode(
        OperationParams({
  ...
  function closeLeveragePosition(
    ...
    UserPosition memory position = positions[
      mapPositions[encodeAddresses(msg.sender, collateralToken, borrowToken)]
    ];
    ...
    flashLoanController.executeFlashLoan(
      ...
      abi.encode(
        OperationParams({
          positionId: mapPositions[encodeAddresses(msg.sender, collateralToken, borrowToken)],
          user: msg.sender,
  ...
  function updateLeveragePosition(
    ...
    uint256 positionId = mapPositions[encodeAddresses(msg.sender, collateralToken, borrowToken)];
    ...
    positions[positionId].initialCollateral = newInitialCollateral;
  ...
  function _executeOpenPosition(
    ...
    uint256 expectedAmountOut = params.initialCollateral * (params.leverageMultiplier - 1);
    ...
    uint256 _totalCollateral = params.initialCollateral + swappedAmount;
    IERC20(params.collateralToken).approve(address(lendingPool), _totalCollateral);
    lendingPool.deposit(params.collateralToken, _totalCollateral, params.user, REFERRAL_CODE);
    ...
    userPositionIndex[params.user][params.positionId] = userPositions[params.user].length - 1;
    mapPositions[encodeAddresses(params.user, params.collateralToken, params.borrowToken)] = params
      .positionId;
  ...
  function _executeClosePosition(
    ...
    uint256 expectedOutput = swapController.getQuote(
      position.collateralToken,
      position.borrowToken,
      withdrawnAmount,
      DEFAULT_POOL_FEE
    ...
    // Calculate proportion of collateral needed for flash loan repayment
    uint256 collateralForRepayment = (withdrawnAmount * amountOwed) / expectedOutput;
    uint256 remainingCollateral = withdrawnAmount - collateralForRepayment;

    // Swap only the necessary amount for flash loan repayment
    IERC20(position.collateralToken).approve(address(swapController), collateralForRepayment);
    swapController.swap(
      position.collateralToken,
      position.borrowToken,
      collateralForRepayment,
      amountOwed, // exact amount needed for flash loan
      DEFAULT_POOL_FEE
    );
    ...
    // Return remaining collateral directly to user
    ...
    delete mapPositions[encodeAddresses(params.user, params.collateralToken, params.borrowToken)];
  ...
  function _removeUserPosition(address user, uint256 positionId) internal {
    ...
    uint256 lastId = userPos[userPos.length - 1];

    userPos[index] = lastId;
    userPositionIndex[user][lastId] = index;
    userPos.pop();
  ...
  /// @notice ABI‑encodes three addresses into a `bytes` blob (with 32‑byte slots)
  function encodeAddresses(
    address user,
    address collateral,
    address borrow
  ) public pure returns (bytes memory) {
    return abi.encode(user, collateral, borrow);
  }
  ...
  function _executeUpdateLeverage(
    ...
    if (params.leverageMultiplier > position.leverageMultiplier) {
      ...
      IERC20(params.borrowToken).approve(address(swapController), amount);
      uint256 expectedAmountOut = params.initialCollateral *
        (params.leverageMultiplier - position.leverageMultiplier);
      uint256 minAmountOut = _calculateMinAmountOut(expectedAmountOut);
      ...
      collateralToWithdraw = _calculateMaxAmountOut(collateralToWithdraw);
      IERC20(reserveData.aTokenAddress).transferFrom(
        params.user,
        address(this),
        collateralToWithdraw
  ...
  function batchAddAllowedCollateralTokens(address[] memory tokens) external onlyOwner {
    for (uint256 i = 0; i < tokens.length; i++) {
      allowedCollateralTokens[tokens[i]] = true;
    }
  }

  function batchAddAllowedBorrowTokens(address[] memory tokens) external onlyOwner {
    for (uint256 i = 0; i < tokens.length; i++) {
      allowedBorrowTokens[tokens[i]] = true;
    }
  }
  ...
  function updateSlippage(uint24 _slippage) external onlyOwner {
