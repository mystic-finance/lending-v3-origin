...
import {IERC20Permit} from '../dependencies/openzeppelin/contracts/IERC20Permit.sol';
import {MarketConfig, PendingUint192, PendingAddress} from '../protocol/libraries/vault/PendingLib.sol';
...
/// @dev This interface is used for factorizing IVaultStaticTyping and IVault.
/// @dev Consider using the IVault interface instead of this one.
...
/// @dev This interface is inherited by Vault so that function signatures are checked by the compiler.
/// @dev Consider using the IVault interface instead of this one.
interface IVaultStaticTyping is IVaultBase {
...
/// @dev Use this interface for Vault to have access to all the functions with the appropriate function signatures.
interface IVault is IVaultBase, IERC4626, IERC20Permit, IOwnable, IMulticall {
  /// @notice Returns the current configuration of each market.
