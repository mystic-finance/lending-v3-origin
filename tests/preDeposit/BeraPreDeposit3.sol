...
contract StoneBeraVaultTest3 is Test {
  ...
  address deployer = 0x18E1EEC9Fa5D77E472945FE0d48755386f28443c; //msg.sender

  function setUp() public {
    console.log('Deployer: %s', msg.sender);

    lpToken = Token(0x04e2f381fEE12b0930bFDE5be98a735C9b448BBB);
    console.log('LP Token Address: %s', address(lpToken));
    tokenA = MockToken(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
    tokenB = MockToken(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
    stoneBeraVault = StoneBeraVault(0x56eE018B23A840cDDa31BDc8fEFCb4be3F0d396D);
    // tokenA.mint(msg.sender, 100e18);
    // deal(address(tokenA), msg.sender, 100e18);
    deal(address(tokenB), deployer, 100e18);
    // tokenB.mint(msg.sender, 100e18);
  ...
  function test_deposit_basic_b() public {
    ...
    stoneBeraVault.deposit(address(tokenB), 1e18, deployer);
    assertEq(tokenB.balanceOf(address(stoneBeraVault)), 1e18);
    assertEq(lpToken.balanceOf(address(stoneBeraVault)), 0);
    // assertEq(lpToken.balanceOf(deployer), 1e18);

    // assertEq(stoneBeraVault.getRate(), 1e18);
    stoneBeraVault.withdrawAssets(address(tokenB), 1e17);
    assertEq(tokenB.balanceOf(address(stoneBeraVault)), 9e17);
    assertEq(tokenB.balanceOf(address(deployer)), 1e17);
    assertEq(lpToken.balanceOf(address(stoneBeraVault)), 0);

    tokenB.approve(address(stoneBeraVault), 2e18);
    stoneBeraVault.deposit(address(tokenB), 2e18, address(this));
    assertEq(lpToken.balanceOf(address(this)), 2e18);
  ...
  function test_deposit_diff_A_price() public {
    ...
    assertEq(lpToken.balanceOf(bob), uint256(1e18).mulDiv(1e18, price, Math.Rounding.Floor));
    assertEq(stoneBeraVault.getRate(), price);
  }
  ...
  function test_deposit_all_diff_A_price() public {
    ...
    assertEq(stoneBeraVault.getRate(), (price1 * 2e18 + 1e18 * 1e18) / (1e18 + 2 * price));
    ...
    assertEq(
      ...
      1e18 + price + price1.mulDiv(1e18, stoneBeraVault.getRate(), Math.Rounding.Floor)
    );
  }
  ...
  function test_roll_multiple_diff_B_price() public {
    ...
    vm.startPrank(alice);
    uint256 redeemable = redeemAmount.mulDiv(rate, price, Math.Rounding.Floor);
  ...
  function test_wrapper() public {
    ...
    DepositWrapper wrapper = new DepositWrapper(address(weth), address(stoneBeraVault));
