...
contract PermPoolTests is PoolTests {
  ...
  function test_l2_supply_permit(uint128 userPk, uint128 supplyAmount) public {
    ...
    vm.prank(poolAdmin);
    portal.addPoolUsers(user, 0, IPoolAddressesProvider(report.poolAddressesProvider));
  ...
  function test_l2_swap_borrow_rate() public {
    ...
    // readd as pool user and retes errors
    vm.stopPrank();
  }

  function test_l2_rebalance_borrow() public {
    vm.prank(poolAdmin);
    portal.addPoolUsers(address(this), 0, IPoolAddressesProvider(report.poolAddressesProvider));
  ...
  function test_l2_liquidationCall() public {
    ...
    portal.addPartner(carol, IPoolAddressesProvider(report.poolAddressesProvider));
    portal.addPoolUsers(alice, 0, IPoolAddressesProvider(report.poolAddressesProvider));
    vm.stopPrank();

    vm.startPrank(carol);
    portal.addLiquidator(
      KYCPortal.Liquidator(0, bob, address(0)),
    ...
    // remove carol
    vm.prank(poolAdmin);
    portal.removePoolUsers(carol, 0, IPoolAddressesProvider(report.poolAddressesProvider));
